...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	// save the system call, we're gonna need it later
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
	...
	// See req (a) in above comments
	if (syscall < 0 
	|| syscall > NR_syscalls 
	|| syscall == MY_CUSTOM_SYSCALL) {
		return -EINVAL;
	}
	// See req (b) in above comments
	if (pid < 1 // pids are non negative and 0 is for monitor all
...
static int init_function(void) {
	...
	// This function still needs synchronization but im not sure exactly how to sync it lol
	...
	//table = kmalloc((NR_syscalls + 1) * sizeof(mytable), __GFP_NORETRY);
	for (i = 0; i < NR_syscalls + 1; i ++) {
		// This struct tracks a the interception and other things about
		// a syscall. Each syscall gets its own instance of this struct.
		mytable sys_call_tracker = 
		{
			sys_call_table[i], // The original function to call
			0, // function has not been intercepted so this is 0,
			0, // no pids being monitored, this is 0,
			0, // there are no pids being minitored, this is 0
		};
		// Initialize my_list variable in struct
		INIT_LIST_HEAD(&(sys_call_tracker.my_list));
		table[i] = sys_call_tracker;
	}
	return 0;
}
