...
int main(int argc, char **argv) {
	...
	// I (Kemar) changed this line, it didnt have sudo before -KH
	test("sudo insmod interceptor.ko %s", "", system("sudo insmod interceptor.ko") == 0);
	...
	// I changed both these lines, from __NR_exit to __NR_exit_group -KH
	do_intercept(__NR_exit_group, 0);
	do_release(__NR_exit_group, 0);
	...
	// I also added the sudo command  here -KH
	test("sudo rmmod interceptor.ko %s", "", system("sudo rmmod interceptor") == 0);
	return 0;
}
